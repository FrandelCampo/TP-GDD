...
AS
	...
	CREATE NONCLUSTERED INDEX IX_TipoInmueble ON PIE_DERECHO.tipos_de_inmueble (TIPO_INMUEBLE_INMUEBLE);
    CREATE NONCLUSTERED INDEX IX_Disposicion ON PIE_DERECHO.disposiciones (DISPOSICION_INMUEBLE);
    CREATE NONCLUSTERED INDEX IX_Ambientes ON PIE_DERECHO.ambientes (AMBIENTES_CANT_INMUEBLE);
    CREATE NONCLUSTERED INDEX IX_Orientacion ON PIE_DERECHO.orientaciones (ORIENTACION_INMUEBLE);
    CREATE NONCLUSTERED INDEX IX_Estados ON PIE_DERECHO.estados (ESTADO_INMUEBLE);
    CREATE NONCLUSTERED INDEX IX_Ubicacion ON PIE_DERECHO.ubicaciones (UBICACION_DIRECCION);
    CREATE NONCLUSTERED INDEX IX_Propietarios ON PIE_DERECHO.propietarios (PROPIETARIO_DNI);
...
DROP INDEX PIE_DERECHO.tipos_de_inmueble.IX_TipoInmueble;
DROP INDEX PIE_DERECHO.disposiciones.IX_Disposicion;
DROP INDEX PIE_DERECHO.ambientes.IX_Ambientes;
DROP INDEX PIE_DERECHO.orientaciones.IX_Orientacion;
DROP INDEX PIE_DERECHO.estados.IX_Estados;
DROP INDEX PIE_DERECHO.ubicaciones.IX_Ubicacion;
DROP INDEX PIE_DERECHO.propietarios.IX_Propietarios;
...
AS
	BEGIN
	INSERT INTO PIE_DERECHO.sucursales (SUCURSAL_CODIGO, SUCURSAL_NOMBRE, SUCURSAL_TELEFONO, SUCURSAL_UBICACION_CODIGO, SUCURSALES_DUPLICADO)
	SELECT DISTINCT M.SUCURSAL_CODIGO, M.SUCURSAL_NOMBRE, M.SUCURSAL_TELEFONO, U.UBICACION_CODIGO, 0
	FROM gd_esquema.Maestra M
	JOIN ubicaciones U ON U.UBICACION_DIRECCION = M.INMUEBLE_DIRECCION
	WHERE M.SUCURSAL_CODIGO IS NOT NULL

	UPDATE PIE_DERECHO.sucursales
	SET SUCURSALES_DUPLICADO = 1
	WHERE PIE_DERECHO.sucursales.SUCURSAL_CODIGO IN (
    SELECT PIE_DERECHO.sucursales.SUCURSAL_CODIGO
    FROM PIE_DERECHO.sucursales
    GROUP BY PIE_DERECHO.sucursales.SUCURSAL_CODIGO
    HAVING COUNT(*) > 1
	)
...
AS
	...
	CREATE INDEX IX_TipoOperacion ON PIE_DERECHO.tipos_de_operacion (TIPO_OPERACION_ANUNCIO);
	CREATE INDEX IX_Moneda ON PIE_DERECHO.monedas (MONEDA_NOMBRE);
	CREATE INDEX IX_AgenteDni ON PIE_DERECHO.agentes (AGENTE_DNI);

	INSERT INTO PIE_DERECHO.anuncios (ANUNCIO_CODIGO, ANUNCIO_FECHA_PUBLICACION, ANUNCIO_PRECIO_PUBLICADO, ANUNCIO_COSTO_ANUNCIO, ANUNCIO_FECHA_FINALIZACION, ANUNCIO_ESTADO, ANUNCIO_TIPO_PERIODO, ANUNCIO_INMUEBLE_PK, ANUNCIO_DUPLICADO, ANUNCIO_TIPO_OPERACION_CODIGO, ANUNCIO_MONEDA_CODIGO, ANUNCIO_AGENTE_CODIGO)
	SELECT DISTINCT M.ANUNCIO_CODIGO, M.ANUNCIO_FECHA_PUBLICACION, M.ANUNCIO_PRECIO_PUBLICADO, M.ANUNCIO_COSTO_ANUNCIO, M.ANUNCIO_FECHA_FINALIZACION, M.ANUNCIO_ESTADO, M.ANUNCIO_TIPO_PERIODO, I.INMUEBLE_PK, 0, T.TIPO_OPERACION_CODIGO, MO.MONEDA_CODIGO, A.AGENTE_CODIGO
	FROM gd_esquema.Maestra M
	...
	UPDATE PIE_DERECHO.anuncios
	SET ANUNCIO_DUPLICADO = 1
	WHERE PIE_DERECHO.anuncios.ANUNCIO_CODIGO IN (
    SELECT PIE_DERECHO.anuncios.ANUNCIO_CODIGO
    FROM PIE_DERECHO.anuncios
    GROUP BY PIE_DERECHO.anuncios.ANUNCIO_CODIGO
    HAVING COUNT(*) > 1
	)
...
AS
	...
	CREATE INDEX IX_EstadoDeAlquileres ON PIE_DERECHO.estado_de_alquileres (ESTADO_DE_ALQUILERES_NOMBRE);
	CREATE INDEX IX_InquilinosDni ON PIE_DERECHO.inquilinos (INQUILINO_DNI);

	INSERT INTO PIE_DERECHO.alquileres (ALQUILER_CODIGO, ALQUILER_FECHA_INICIO, ALQUILER_FECHA_FIN, ALQUILER_CANT_PERIODOS, ALQUILER_DEPOSITO, ALQUILER_COMISION, ALQUILER_GASTOS_AVERIGUA, ALQUILER_VENTA_ANUNCIO_PK, ALQUILER_DUPLICADO,ALQUILER_ESTADO_DE_ALQUILERES_CODIGO, ALQUILER_INQUILINO_CODIGO)
	SELECT DISTINCT M.ALQUILER_CODIGO, M.ALQUILER_FECHA_INICIO, M.ALQUILER_FECHA_FIN, M.ALQUILER_CANT_PERIODOS, M.ALQUILER_DEPOSITO, M.ALQUILER_COMISION, M.ALQUILER_GASTOS_AVERIGUA, A.ANUNCIO_PK, 0,
	...
	UPDATE PIE_DERECHO.alquileres
	SET ALQUILER_DUPLICADO = 1
	WHERE PIE_DERECHO.alquileres.ALQUILER_CODIGO IN (
    SELECT PIE_DERECHO.alquileres.ALQUILER_CODIGO
    FROM PIE_DERECHO.alquileres
    GROUP BY PIE_DERECHO.alquileres.ALQUILER_CODIGO
    HAVING COUNT(*) > 1
	)
...
AS 
	...
	SELECT DISTINCT M.DETALLE_ALQ_NRO_PERIODO_INI, M.DETALLE_ALQ_NRO_PERIODO_FIN, M.DETALLE_ALQ_PRECIO, A.ALQUILER_CODIGO
	FROM gd_esquema.Maestra M
...
AS
	...
	CREATE NONCLUSTERED INDEX IX_MediosDePgo ON PIE_DERECHO.medios_de_pago (MEDIOS_DE_PAGO_NOMBRE);
	...
	JOIN medios_de_pago MP ON MP.MEDIOS_DE_PAGO_NOMBRE = M.PAGO_VENTA_MEDIO_PAGO
...
AS
	...
	INSERT INTO PIE_DERECHO.ventas (VENTA_CODIGO, VENTA_FECHA, VENTA_PRECIO_VENTA, VENTA_COMISION, VENTA_ANUNCIO_PK, VENTA_COMPRADOR_CODIGO, VENTA_DUPLICADO)
	SELECT DISTINCT M.VENTA_CODIGO, M.VENTA_FECHA, M.VENTA_PRECIO_VENTA, M.VENTA_COMISION, A.ANUNCIO_PK, C.COMPRADOR_CODIGO, 0
	FROM gd_esquema.Maestra M
	JOIN anuncios A ON A.ANUNCIO_CODIGO = M.ANUNCIO_CODIGO
	JOIN compradores C ON C.COMPRADOR_DNI = M.COMPRADOR_DNI
	WHERE M.VENTA_CODIGO IS NOT NULL

	UPDATE PIE_DERECHO.ventas
	SET VENTA_DUPLICADO = 1
	WHERE PIE_DERECHO.ventas.VENTA_CODIGO IN (
    SELECT PIE_DERECHO.ventas.VENTA_CODIGO
    FROM PIE_DERECHO.ventas
    GROUP BY PIE_DERECHO.ventas.VENTA_CODIGO
    HAVING COUNT(*) > 1
	)
...
SELECT * FROM PIE_DERECHO.alquileres
